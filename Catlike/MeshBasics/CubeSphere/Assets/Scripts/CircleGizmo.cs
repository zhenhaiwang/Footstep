...
public class CircleGizmo : MonoBehaviour
{
    ...
    private void ShowPoint(float x, float y)
    {
        ...
        // For each point we will show the corresponding circle vertex, using a white sphere.
        Gizmos.color = Color.white;
        Gizmos.DrawSphere(circle, 0.025f);
        // Then we show the relationship between these two vertices with a yellow line.
        Gizmos.color = Color.yellow;
        Gizmos.DrawLine(square, circle);
        // And finally a gray line from the circle vertex to the center.
